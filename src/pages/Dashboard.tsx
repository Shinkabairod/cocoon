...
const Dashboard = () => {
  ...
  const renderResourcesPage = () => <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowNewFolderModal(true)} className="bg-black text-white hover:bg-gray-800 transition-colors">
      ...
      <div className="flex gap-2">
        <Button variant={activeCategory === 'resources' ? 'default' : 'outline'} onClick={() => setActiveCategory('resources')} className={`gap-2 ${activeCategory === 'resources' ? 'bg-black text-white' : ''}`}>
          <Folder className="h-4 w-4" />
          Resources
        </Button>
        <Button variant={activeCategory === 'personal' ? 'default' : 'outline'} onClick={() => setActiveCategory('personal')} className={`gap-2 ${activeCategory === 'personal' ? 'bg-black text-white' : ''}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {folders[activeCategory].map(folder => <div key={folder.id} className="bg-gradient-to-br from-violet-50/50 to-purple-50/50 border border-violet-200/30 rounded-xl p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-1 cursor-pointer backdrop-blur-sm">
            <div className="text-center">
  ...
  const addItem = async () => {
    ...
    try {
      ...
      setFolders(prev => ({
        ...prev,
        [activeCategory]: prev[activeCategory].map(folder => folder.id === selectedFolder.id ? {
          ...folder,
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="flex items-center justify-between h-16 px-6">
          {/* Logo et menu mobile */}
          <div className="flex items-center gap-4">
            {isMobile && <Button variant="ghost" size="sm" onClick={() => setSidebarOpen(true)}>
          ...
          <div className="hidden md:flex items-center flex-1 max-w-md mx-6">
            <div className="relative w-full">
              ...
              <input type="text" placeholder="Search..." className="w-full pl-10 pr-4 py-2 bg-gray-100 border-none rounded-lg focus:outline-none focus:ring-2 focus:ring-black focus:bg-white transition-all" />
            </div>
          </div>

          {/* Actions utilisateur */}
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => setActiveModal('chat')} className="relative">
      ...
      <div className="flex">
        {/* Sidebar Desktop */}
        {!isMobile && <aside className="w-64 bg-white border-r border-gray-200 h-[calc(100vh-64px)] sticky top-16 flex flex-col">
            <nav className="flex-1 p-4">
              <div className="space-y-1">
                {navItems.map(item => <button key={item.id} onClick={() => setActivePage(item.id)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-all ${activePage === item.id ? 'bg-black text-white' : 'text-gray-700 hover:bg-gray-100'}`}>
              ...
              <div className="mt-8">
                ...
                <div className="space-y-1">
                  {accountItems.map((item, index) => <button key={index} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left text-gray-700 hover:bg-gray-100 transition-all">
                      <item.icon className="h-5 w-5" />
                      <span className="font-medium">{item.label}</span>
                    </button>)}
                </div>
        ...
        {/* Sidebar Mobile */}
        {isMobile && sidebarOpen && <div className="fixed inset-0 z-50">
            ...
            <aside className="fixed left-0 top-0 h-full w-64 bg-white shadow-xl">
              ...
              }} className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-all ${activePage === item.id ? 'bg-black text-white' : 'text-gray-700 hover:bg-gray-100'}`}>
                      <item.icon className="h-5 w-5" />
                      <span className="font-medium">{item.label}</span>
                    </button>)}
                </div>
      ...
      {/* Modal Chat IA */}
      {activeModal === 'chat' && <Dialog open={true} onOpenChange={() => setActiveModal(null)}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 max-h-96 overflow-y-auto">
                {chatMessages.length > 0 ? <div className="space-y-4">
                    {chatMessages.map((message, index) => <div key={index} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${message.type === 'user' ? 'bg-black text-white' : 'bg-white border shadow-sm'}`}>
                          {message.content}
                        </div>
                      </div>)}
              ...
              {/* Input */}
              <div className="flex gap-2">
                <Input placeholder="Type your message..." value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSendMessage()} className="flex-1" />
                <Button onClick={handleSendMessage} disabled={!chatInput.trim()} className="bg-black text-white hover:bg-gray-800">
      ...
      {showNewFolderModal && <Dialog open={true} onOpenChange={() => setShowNewFolderModal(false)}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label>Emoji</Label>
                <Input value={newFolderEmoji} onChange={e => setNewFolderEmoji(e.target.value)} placeholder="📁" />
              </div>
              <div className="flex gap-2">
                <Button onClick={() => {
      ...
      {showNewItemModal && <Dialog open={true} onOpenChange={() => setShowNewItemModal(false)}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="itemName" placeholder="Item name" value={newItemData.name} onChange={e => setNewItemData(prev => ({
              ...prev,
              name: e.target.value
            }))} />
              </div>
              ...
              <div>
                ...
                <Select value={newItemData.type} onValueChange={value => setNewItemData(prev => ({
              ...prev,
              type: value
            }))}>
                  <SelectTrigger>
              ...
              {newItemData.type === 'text' && <div>
                  <Label htmlFor="itemContent">Content</Label>
                  <Textarea id="itemContent" placeholder="Enter your text content..." value={newItemData.content} onChange={e => setNewItemData(prev => ({
              ...prev,
              content: e.target.value
            }))} rows={4} />
                </div>}
              
              {newItemData.type === 'file' && <div>
                  <Label htmlFor="itemFile">File</Label>
                  <Input id="itemFile" type="file" onChange={handleFileUpload} />
                  {newItemData.file && <div className="mt-2 p-2 bg-gray-50 rounded text-sm">
                      Selected file: {newItemData.file.name}
                    </div>}
                </div>}
              
              {newItemData.type === 'link' && <div>
                  <Label htmlFor="itemUrl">URL</Label>
                  <Input id="itemUrl" type="url" placeholder="https://..." value={newItemData.url} onChange={e => setNewItemData(prev => ({
              ...prev,
              url: e.target.value
            }))} />
                </div>}
